 ...
 * @author rekgnyz
 */
    public class ImportRoads  extends JInternalFrame{
    public ImportRoads(Project p) {
           
        JFileChooser chooser = new JFileChooser();
        chooser.setDialogTitle("Import Roads To Existing Project");
        FileNameExtensionFilter filter = new FileNameExtensionFilter("XML Files", "xml");
        chooser.addChoosableFileFilter(filter);
        chooser.setAcceptAllFileFilterUsed(false); 
        //chooser.setFileFilter(filter);
        int option = chooser.showOpenDialog(ImportRoads.this);
        if (option == JFileChooser.APPROVE_OPTION) {
            try{
                ...
                Project pj2 = estrategia.importNetworkFile(fi);
                p.setSections(pj2.getSections()); 
