 ...
 * @author suq-madik
 */
public class ImportVehicles extends JInternalFrame{
    public ImportVehicles(Project p) {
           
        JFileChooser chooser = new JFileChooser();
        chooser.setDialogTitle("Import Vehicles File");
        FileNameExtensionFilter filter = new FileNameExtensionFilter("XML Files", "xml");
        chooser.addChoosableFileFilter(filter);
        chooser.setAcceptAllFileFilterUsed(false); 
        //chooser.setFileFilter(filter);
        int option = chooser.showOpenDialog(ImportVehicles.this);
        if (option == JFileChooser.APPROVE_OPTION) {
            try{
                ...
                ArrayList<Vehicle> vehicles = estrategia.importVehiclesFile(fi);
